...
def smooth_per_group(df, sigma, columns_to_smooth):
    """
    One-sided Gaussian smoother, NaN-aware.

    Script 3 now reindexes the panel to a complete ward x month grid, so months
    with no MUAC collection are present as rows with NaN outcomes. np.dot
    returns NaN if any value in the window is NaN, which would propagate each
    gap forward across the whole 9-month window - roughly 900 lost target
    values from about 110 gap rows.

    The kernel is therefore restricted to the observed months in the window and
    renormalised, so a gap reweights the average rather than destroying it.
    This is also more correct than the previous behaviour, which treated the
    months either side of a gap as if they were consecutive.

    smoothed_values is also allocated as float. np.zeros_like on an integer
    column would have silently truncated every smoothed value to an integer.
    """
    smoothed_df = df.copy()
    ...
    for unit_id in df['Ward'].unique():
        ...
        for col in columns_to_smooth:
            ...
            vals = unit_data[col].to_numpy(dtype=float)
            smoothed_values = np.full(len(vals), np.nan)
            ...
            for i in range(len(unit_data)):
                ...
                window = vals[i - current_window_size + 1 : i + 1]
                observed = ~np.isnan(window)
                if not observed.any():
                    continue                                    # whole window missing
                k = kernel[observed] / kernel[observed].sum()   # renormalise over observed
                smoothed_values[i] = np.dot(window[observed], k)
...
#===========================================
# FIX A: script 3 emits these as "<var>_<season>_<stat>_prev", with season in
# {season1, season2} and stat in {max, sum, mean}. season1 = Mar-May (long
# rains), season2 = Oct-Dec (short rains). The tokens searched for previously
# (long_rain/short_rain, max_prev/total_prev/avg_prev) exist nowhere in the
# dataset, so both lists came back empty and every seasonal feature was silently
# dropped from hb_m1/m2/m3. The hb_m* definitions below are unchanged.
SEASON_MAP = {'long_rain': 'season1', 'short_rain': 'season2'}

for rain_type, season in SEASON_MAP.items():
    globals()[rain_type] = [
        var for var in data
        if f"_{season}_" in var and var.endswith("_prev")
    ]
    globals()[rain_type + '_lag'] = [
        var for var in data
        if f"_{season}_" in var and var.endswith("_lag")
    ]

for rain_type in SEASON_MAP:
    if not globals()[rain_type]:
        raise RuntimeError(
            f"Feature list '{rain_type}' is empty - no seasonal columns found in "
            f"the dataset. Check generate_seasonal_features_and_lags in script 3."
        )
    print(f"{rain_type}: {len(globals()[rain_type])} seasonal features")
...
# INCLUDE_SEASONAL controls whether the seasonal aggregates enter the model.
# It defaults to False so that this script reproduces the configuration the
# backtest history was generated under. The seasonal features have never been
# validated against outturns - set True only alongside a fresh backtest, or the
# forward forecast will be inconsistent with the historical series it is
# appended to.
INCLUDE_SEASONAL = False
seasonal = (long_rain + short_rain) if INCLUDE_SEASONAL else []
print(f"seasonal features used in the model: {len(seasonal)} "
      f"(available: {len(long_rain) + len(short_rain)})")

hb_m1 = static + seasonal + l1 + l2 + l3
hb_m2 = static + seasonal + l2 + l3 + l4
hb_m3 = static + seasonal + l3 + l4 + l5

# The smoothed outcome lags overlap the target by construction: the target is a
# one-sided Gaussian average over the previous 9 months, so wasting_sm_lag_1
# shares 8 of those 9 months with it. That inflates apparent skill at short
# horizons without improving the forecast. Dropping them cost 0.06 of
# within-ward R2 at h=1, GAINED at h=2 and h=3, and removed a systematic
# positive bias of about +0.005 at h=3.
DROP_SMOOTHED_LAGS = True

if DROP_SMOOTHED_LAGS:
    for _name in ("hb_m1", "hb_m2", "hb_m3"):
        _before = len(globals()[_name])
        globals()[_name] = [c for c in globals()[_name] if "_sm_" not in c]
        print(f"  {_name}: {_before} -> {len(globals()[_name])} features "
              f"(smoothed outcome lags dropped)")
...
def bootstrap_iteration(model,train, testX, ff_m, num_iterations):
    ...
    def single_bootstrap_iteration(iteration):
        # FIX C: identical stratified resample, without the pandas deprecation
        # that changes `include_groups` behaviour in 3.0. Also faster.
        resampled_data = train.groupby('time_cont_enc', group_keys=False).sample(
            frac=1, replace=True)
...
def train_test_split(data_train, data_test, n_test, pred_h):
    ...
    # FIX B: was `data[mask]`, silently reading the module-level frame instead
    # of the argument. Same result at the current call site; wrong if reused.
    return data_train[mask], data_test[data_test['time_cont_enc'] == t]
#---------------------------------------------------------------------
...
def merge_and_replace_predictions(
    ...
    # --- Prepare last observed date & map ---
    last_obs_date = None
    obs_last_map = None
    ...
    for h in horizons:
        ...
        # Load future file first to know what date range we're creating
        ...
        # Get the expected new filename BEFORE we filter candidates
        future_min = df_future["time_period"].min()
        future_max = df_future["time_period"].max()
        expected_new_filename = f"{outcome_var}_pred_{model}_{h}_36m_{future_min.year}_{future_min.month:02d}_to_{future_max.year}_{future_max.month:02d}.csv"
        
        # Filter historic candidates, excluding:
        # 1. future_months files
        # 2. The file we're about to create (in case of re-runs)
        historic_candidates = []
        for f in all_files:
            if not (pattern_with_to.fullmatch(f) or pattern_no_to.fullmatch(f)):
                continue
            if f.endswith("_future_months.csv"):
                continue
            if f == expected_new_filename:
                print(f"⚠️ Skipping {f} as it matches the new filename we're creating")
                continue
            historic_candidates.append(f)

        # If no historic: save future-only
        if not historic_candidates:
            ...
            # USE ZERO-PADDED MONTHS
            new_filename = f"{outcome_var}_pred_{model}_{h}_36m_{min_date.year}_{min_date.month:02d}_to_{max_date.year}_{max_date.month:02d}.csv"
        ...
        if last_obs_date is not None and obs_last_map is not None:
            ...
            if is_last_month.any():
                ...
                print(f"✔️ Backfilled '{outcome_var}' for last observed month {last_obs_date.date()}")

        # ---- Keep future on overlap ----
        ...
        # Merge & write.
        # Development runs left two rows for the same (Ward, time_period,
        # time_horizon) at several 2025 origins, from different model fits with
        # different training sets. A reader that does not deduplicate shows
        # whichever row comes first. _src makes "keep the newer forecast"
        # deterministic rather than dependent on concat order.
        df_hist_no_overlap = df_hist_no_overlap.assign(_src=0)
        df_future = df_future.assign(_src=1)
        df_merged = pd.concat([df_hist_no_overlap, df_future], ignore_index=True)
        df_merged = df_merged.sort_values(
            [ward_col, "time_period", "time_horizon", "_src"])

        _before = len(df_merged)
        df_merged = df_merged.drop_duplicates(
            subset=[ward_col, "time_period", "time_horizon"], keep="last")
        if len(df_merged) < _before:
            print(f"[!] dropped {_before - len(df_merged)} duplicate keys for "
                  f"h={h}, kept the later forecast")

        df_merged = df_merged.drop(columns="_src").reset_index(drop=True)

        min_date = df_merged["time_period"].min()
        max_date = df_merged["time_period"].max()
        # USE ZERO-PADDED MONTHS - THIS IS KEY!
        new_filename = f"{outcome_var}_pred_{model}_{h}_36m_{min_date.year}_{min_date.month:02d}_to_{max_date.year}_{max_date.month:02d}.csv"
        ...
        if delete_inputs:
            ...
            for f in (historic_file, future_file):
                if os.path.abspath(f) == os.path.abspath(new_path):
                    print(f"⏭️  Not deleting {os.path.basename(f)} - it IS the output")
                    continue
                try:
                    os.remove(f)
    ...
    for m in model_name:
        for horizon in ph:
            ...
            if feat_list_name not in globals():
                ...
                continue
            # FIX D: this filter silently discarded any requested feature that
            # was absent from the dataset, which is how 114 missing columns went
            # unnoticed. Fail instead.
            requested = globals()[feat_list_name]
            feat_list = [c for c in requested if c in data.columns]
            missing = [c for c in requested if c not in data.columns]
            if missing:
                raise RuntimeError(
                    f"{len(missing)} features in {feat_list_name} are absent from "
                    f"the dataset: {missing[:10]}"
                )
            print(f"{feat_list_name}: {len(feat_list)} features")

            # Anchor on the RAW MUAC column, not the smoothed target. The
            # NaN-aware smoother fills a value for any row with observed months
            # in its window, including the future rows created by
            # extend_future_muac_periods and the gap rows from
            # reindex_complete_panel. Neither has a measurement behind it, so
            # using the smoothed column pushes the origin past the end of real
            # data and forecasts from a phantom month.
            _raw_col = ("wasting" if outcome_var.startswith("wasting_smoothed")
                        else "wasting_risk")
            _raw_mask = (data_val[_raw_col].notna() if _raw_col in data_val.columns
                         else mask)
            last_obs_code = data_val.loc[_raw_mask, "time_cont_enc"].max()
            ...
            # Gap rows carry a smoothed target derived entirely from
            # neighbouring months, with no measurement in it - training on them
            # fits the model to a value the smoother built from the same lags
            # that are already features. They stay in the test slice, so wards
            # that did not report still get a prediction.
            _gap = train.get("is_gap_row",
                             pd.Series(False, index=train.index)).fillna(False)
            train_clean = train[train[outcome_var].notna() & ~_gap].copy()
            ...
            # A prevalence cannot be negative. Around 45% of lower bounds were
            # below zero before clipping.
            lower_bound = np.maximum(lower_bound, 0.0)
            upper_bound = np.maximum(upper_bound, lower_bound)

            # NOTE ON WHAT THESE INTERVALS MEASURE: the bootstrap resamples
            # training rows and refits, so its spread is PARAMETER uncertainty -
            # how much the fitted model moves with different training data. It
            # excludes the irreducible error of predicting one ward-month, so it
            # is a confidence interval for the expected prediction rather than a
            # prediction interval. Backtest coverage is 75-79% against a nominal
            # 90%, with 14-19% of outturns falling BELOW the lower bound. Do not
            # present these as 90% intervals.
...
TOKEN_MAP = {
    ...
    # temperature counts: nonparametric standardised index, not z-scores
    "hot_days_ns":                "Hot days, standardised",
    "cold_days_ns":               "Cold days, standardised",
    "consec_hot_days_ns":         "Longest hot spell, standardised",
    "consec_cold_days_ns":        "Longest cold spell, standardised",
    "hot_days":                   "Hot days",
    "cold_days":                  "Cold days",
    "consec_hot_days":            "Longest hot spell (days)",
    "consec_cold_days":           "Longest cold spell (days)",

    # precipitation: SPI and nonparametric standardised indices
    "spi_1":                      "Rainfall anomaly, 1 month (SPI-1)",
    "spi_3":                      "Rainfall anomaly, 3 months (SPI-3)",
    "wet_days_ns":                "Wet days, standardised",
    "consec_dry_days_ns":         "Longest dry spell, standardised",
    "precip_total":               "Total precipitation (mm)",
    "wet_days":                   "Wet days",
    "consec_dry_days":            "Longest dry spell (days)",
...
# Config
# =========================
DATA_DIR = OUTPUT       # where your Feature_Importances_*.csv live
...
# FIX E: the two pooled wasting bars absorb ~90% of importance, so TOPK=5 left
# almost no room for covariate families. Display only - no effect on the model.
TOPK = 10
...
#---------------------------
    ...
    DATA_DIR = OUTPUT  # adjust if needed
